...
// 송달료 — 대법원 「송달료규칙」 및 「송달료규칙의 시행에 따른 업무처리요령」(재판예규)
//
//   송달료 = 당사자 수 × 1회 송달료 단가 × 사건별 예납 회차
//
// ⚠️ 아래 값은 Figma 시안(당사자 2명 = 52,000원)에 맞춘 데모 값이고, 법령 대조를 거치지 않았다.
//    · 1회 단가는 우편요금에 연동되어 개정 때마다 바뀐다.
//    · 예납 회차는 사건 종류별로 다르다 (소액·단독·합의·독촉이 각각 다름).
//      소액사건은 통상 당사자수 × 10회분이라, 이 값은 실제보다 적을 가능성이 크다.
//
// 실제 금액은 대법원 전자민원센터(help.scourt.go.kr)의 현재 기준으로 확인해야 한다.
...
const evidenceStep = (options) => ({
  ...
  fields: [
    ...
    checks('evidenceItems', '어떤 자료를 가지고 계신가요?', options, { required: true }),
    note('info', '체크한 자료를 아래에 파일로 올려주세요. 목록에 없는 자료도 그냥 올리시면 됩니다.'),
    ...
    note('info', '증거는 「입증방법」, 나머지 제출 서류는 「첨부서류」로 나뉘어 들어갑니다. 소가계산서는 소가 산정이 복잡한 사건(명도·확인의 소 등)에서 요구될 수 있어요.'),
    note('warn', '전자소송포털은 미리 만든 소장 파일(HWP·WORD)을 첨부하는 방식을 지원하지 않아요. 완성한 소장은 종이 제출용 출력본이자, 전자소송 입력창에 붙여넣을 원문으로 쓰입니다. 증거 파일은 그대로 첨부하시면 돼요.'),
